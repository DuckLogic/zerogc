...
use zerogc::{safepoint_recurse, Gc, GcArray, GcSimpleAlloc};
...
fn simple() {
    ...
    assert_eq!(ctx.alloc(Test { val: 42, rec: None }).val, 42);
}
...
fn recursive() {
    ...
    let first = ctx.alloc(Test { val: 18, rec: None });
...
fn static_alloc() {
    ...
    let array: GcArray<u8, EpsilonCollectorId> = epsilon::gc_array(FOO);
