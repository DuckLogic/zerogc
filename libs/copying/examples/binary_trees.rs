...
use zerogc_copying::{CopyingCollector, CopyingCollectorContext, Gc};
...
fn bottom_up_tree<'gc>(collector: &'gc CopyingCollectorContext, depth: i32)
                       -> Gc<'gc, Tree<'gc>> {
...
fn inner(
    gc: &mut CopyingCollectorContext,
...
fn main() {
    ...
    let collector = CopyingCollector::create();
