...
fn bottom_up_tree<'gc>(collector: &'gc CompactingCollectorContext, depth: i32)
                       -> Gc<'gc, Tree<'gc>> {
...
fn inner(
    gc: &mut CompactingCollectorContext,
...
fn main() {
    ...
    let collector = CompactingCollector::create_compacting();
