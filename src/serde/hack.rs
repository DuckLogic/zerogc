...
impl ContextHackState {
    ...
    unsafe fn lock_unchecked<'gc, C: GcContext>(&self) -> ContextHackGuard<'gc, C> {
        ...
        debug_assert_eq!(
            TypeId::of::<C::Id>(),
            self.get_unchecked().unwrap().collector_type_id
        );
        ContextHackGuard {
...
pub unsafe fn set_context<C: GcContext>(ctx: &C) -> ContextHackGuard<'_, C> {
    let guard = PRIMARY_DE_CONTEXT.with(|state| match state.get_unchecked() {
        Some(other) => {
    ...
    OTHER_DE_CONTEXT.with(|map| {
        ...
        match map.entry(TypeId::of::<C::Id>()) {
            Entry::Occupied(occupied) => {
                ...
                assert_eq!(
                    other.get_unchecked().unwrap().cast_as::<C>().unwrap().id(),
                    ctx.id(),
...
unsafe fn unregister_context(state: &ContextHackState) {
    ...
    if needs_fallback_free {
        OTHER_DE_CONTEXT.with(|map| {
            let mut map = map.borrow_mut();
            let actual_state = map
                .remove(&expected_ctx.collector_type_id)
                .unwrap_or_else(|| unreachable!("Can't find collector in serde::hack state"));
            debug_assert_eq!(
                expected_ctx.collector_type_id,
                actual_state.get_unchecked().unwrap().collector_type_id
            );
            debug_assert_eq!(
                actual_state.get_unchecked().unwrap() as *const _,
                expected_ctx as *const _
            );
...
pub unsafe fn current_ctx<'gc, Id: CollectorId>() -> ContextHackGuard<'gc, Id::Context> {
    PRIMARY_DE_CONTEXT
        .with(|state| match state.get_unchecked() {
...
unsafe fn _fallback_current_ctx<'gc, Id: CollectorId>() -> ContextHackGuard<'gc, Id::Context> {
    OTHER_DE_CONTEXT.with(|map| {
        let map = map.borrow();
        let state = map.get(&TypeId::of::<Id>()).unwrap_or_else(|| {
            unreachable!(
                "Can't find collector for {} in serde::hack state",
...
#[track_caller]
pub unsafe fn unchecked_deserialize_hack<
    'gc,
    'de,
    D: Deserializer<'de>,
    Id: CollectorId,
...
impl<'gc, 'de, Id: CollectorId, T: GcDeserialize<'gc, 'de, Id>> Deserialize<'de>
    for DeserializeHackWrapper<T, Id>
{
        ...
        D: Deserializer<'de>,
    {
        ...
        Ok(DeserializeHackWrapper(
            T::deserialize_gc(unsafe { guard.get_unchecked() }, deserializer)?,
            PhantomData,
        ))
    }
}
